...
static unsigned int OSPI_W25Qxx_ReadID(void);     // 读取器件ID
static char OSPI_W25Qxx_MemoryMappedMode(void);	// 将OSPI设置为内存映射模式
static char OSPI_W25Qxx_SectorErase(unsigned int SectorAddress);        // 扇区擦除，4K字节， 参考擦除时间 45ms
static char OSPI_W25Qxx_BlockErase_32K (unsigned int SectorAddress);    // 块擦除，  32K字节，参考擦除时间 120ms
static char OSPI_W25Qxx_BlockErase_64K (unsigned int SectorAddress);    // 块擦除，  64K字节，参考擦除时间 150ms，实际建议使用64K擦除，擦除的时间最快
static char OSPI_W25Qxx_ChipErase (void);                           // 整片擦除，参考擦除时间 20S
static char OSPI_W25Qxx_WritePage(unsigned char* pBuffer, unsigned int WriteAddr, unsigned short NumByteToWrite);    // 按页写入，最大256字节
static char OSPI_W25Qxx_WriteBuffer(unsigned char* pBuffer, unsigned int WriteAddr, unsigned int Size);            // 写入数据，最大不能超过flash芯片的大小
static char OSPI_W25Qxx_ReadBuffer(unsigned char* pBuffer, unsigned int ReadAddr, unsigned int NumByteToRead);     // 读取数据，最大不能超过flash芯片的大小

 
_w25qx devW25q = { 
	 
	 .ReadId = OSPI_W25Qxx_ReadID,
	 .W25QxxInit = OSPI_W25Qxx_Init,
    .SectorErase = OSPI_W25Qxx_SectorErase,
	 .WritePage   = OSPI_W25Qxx_WritePage,
	 .WriteBufer  = OSPI_W25Qxx_WriteBuffer,
	 .ReadBufer   = OSPI_W25Qxx_ReadBuffer,
	 .MemoryMappedMode = OSPI_W25Qxx_MemoryMappedMode,
	 .BlockErase32K = OSPI_W25Qxx_BlockErase_32K,
	 .BlockErase64K = OSPI_W25Qxx_BlockErase_64K,
	 .ChipErase = OSPI_W25Qxx_ChipErase,
};
...
char OSPI_W25Qxx_Init(void)
{
	uint32_t	Device_ID = 0;	// 器件ID
	
	Device_ID = OSPI_W25Qxx_ReadID(); 		// 读取器件ID
	devW25q.devFlashId = Device_ID;
	if( Device_ID == W25Q128_FLASH_ID || Device_ID == W25Q256_FLASH_ID)		// 进行匹配
	{
...
static char OSPI_W25Qxx_AutoPollingMemReady(void)
{
...
static unsigned int OSPI_W25Qxx_ReadID(void)	
{
	OSPI_RegularCmdTypeDef  sCommand;		// OSPI传输配置
...
static char OSPI_W25Qxx_MemoryMappedMode(void)
{
...
static char OSPI_W25Qxx_WriteEnable(void)
{
   OSPI_RegularCmdTypeDef  sCommand;// OSPI传输配置
   OSPI_AutoPollingTypeDef sConfig;// 轮询比较相关配置参数
...
static char OSPI_W25Qxx_SectorErase(unsigned int SectorAddress)	
{
	OSPI_RegularCmdTypeDef  sCommand;		// OSPI传输配置
...
static char OSPI_W25Qxx_BlockErase_32K (unsigned int SectorAddress)	
{
	OSPI_RegularCmdTypeDef  sCommand;		// OSPI传输配置
...
*
**************************************************************************************************/
static char OSPI_W25Qxx_BlockErase_64K (unsigned int SectorAddress)	
{
	OSPI_RegularCmdTypeDef  sCommand;		// OSPI传输配置
...
*
*************************************************************************************************/
static char OSPI_W25Qxx_ChipErase (void)	
{
	OSPI_RegularCmdTypeDef  sCommand;		// OSPI传输配置
   OSPI_AutoPollingTypeDef sConfig;       // 轮询比较相关配置参数
...
*
***********************************************************************************************************/
static char OSPI_W25Qxx_WritePage(unsigned char* pBuffer, unsigned int WriteAddr, unsigned short NumByteToWrite)
{
   OSPI_RegularCmdTypeDef  sCommand;// OSPI传输配置
...
static char OSPI_W25Qxx_WriteBuffer(unsigned char* pBuffer, unsigned int WriteAddr, unsigned int Size)
{	
...
static char OSPI_W25Qxx_ReadBuffer(unsigned char* pBuffer, unsigned int ReadAddr, unsigned int NumByteToRead)
{
   OSPI_RegularCmdTypeDef  sCommand;// OSPI传输配置
